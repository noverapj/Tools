...
void CBlockingSocket::Create(int nType /* = SOCK_STREAM */)
{
   ...
   // TIME_WAIT 기다리는 시간 설정( 디폴트는 30초인데 5초로 단축 )
	linger optval;
	optval.l_onoff = 1; 
	optval.l_linger= 5; // 5초
	setsockopt( m_hSocket, SOL_SOCKET, SO_LINGER, (char*)&optval, sizeof( optval ) );
...
{
   ASSERT( m_hSocket!=0 );
   if( ::bind(m_hSocket, psa, sizeof(SOCKADDR))==SOCKET_ERROR ) //bind 충돌 수정
   {
