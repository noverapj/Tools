	...
	if (m_pArchive || m_pHeader)
	{
		...
		else 
		{
			...
			m_szInfo.Format(IDS_ARCHINFO, szType, CString((LPCTSTR)(m_pArchive->IsReadOnly() ? IDS_YES : IDS_NO)),
				CZipArcApp::FormatSize(m_pArchive->GetCount()), m_pArchive->GetSegmMode() >= 0 ? m_pArchive->GetCurrentVolume() : info.m_uLastVolume + 1, CZipArcApp::FormatSize(uTotalUncompressed), ratio, 
				CZipArcApp::FormatSize(uTotalDirSize), CString((LPCTSTR)(info.m_bInArchive ? IDS_YES : IDS_NO)));
		}
		m_szInfo.Replace(_T("\n"), _T("\r\n"));
		m_bInfo = false;
	}
