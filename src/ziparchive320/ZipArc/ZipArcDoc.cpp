...
{
	if (m_progress.m_iType == CZipActionCallback::cbSave && m_zip.GetSegmMode()
...
void CZipArcDoc::CProgressData::Finish(bool bModifyMenu)
{
	...
	if (!m_iType || m_iType == CZipActionCallback::cbAdd || m_iType == CZipActionCallback::cbDelete
		|| m_iType == CZipActionCallback::cbRename || m_iType == CZipActionCallback::cbMoveData || m_iType == CZipActionCallback::cbGet)
...
BOOL CZipArcDoc::OpenZipFile(LPCTSTR lpszZipFileName, long iMode, long iVolumeSize) 
{
	...
	do 
	{
		TRY
		{
			...
			_splitpath( lpszZipFileName, NULL, NULL, szLowerFileName, NULL );
			strlwr( szLowerFileName );
			if( strcmp( szLowerFileName , "full" ) != 0 )
			{
				enum LocalType
				{
...
//				LocalType eLocalType = LCT_CHINA;
				...
				char szPassword1[MAX_PATH]="";
				char szPassword2[MAX_PATH]="";

				if( eLocalType == LCT_KOREA )
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "iosuccess#@" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "XrFrI0%3BF%!0Dcx$30-" );
				}
				else if( eLocalType == LCT_US )
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "eE39DkE!%E0" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "Eg%^io03UT$Cvf921-!$" );
				}
				else if( eLocalType == LCT_BRAZIL )
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "jE/6Dy?%E0" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "j%K>?03UT$Cvf921-!$" );
				}
				else if( eLocalType == LCT_TAIWAN )
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "iUT38#@49vnFdjf)(4sg" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "Yi#weT%^903Unv0$2gfj" );
				}
				else if( eLocalType == LCT_INDONESIA )
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "T*$f40FRjfoe*(fl304d" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "Mfe$%2049eFeodk*&31Z" );
				}
				else if( eLocalType == LCT_SINGAPORE )
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "EDgei%^df930%#fj!_=]" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "@7$gjTRreie][!323O++" );
				}
				...
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "K3$dls49YU#$#eoE3054" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "-_495IUEVJdlsl++32ed" );
				}
				else if( eLocalType == LCT_CHINA )
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "-)4TRfkl-41$%dgkrm05" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "|059rtuGReowo@##tkg0" );
				}
				else if( eLocalType == LCT_LATIN )
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "dus!qhdaksl" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "tkdjqqn!dlfEhrqkfhgo" );
				}
				else if( eLocalType == LCT_EU )
				{
					StringCbCopy( szPassword1, sizeof( szPassword1 ), "Efedf12-Asv" );
					StringCbCopy( szPassword2, sizeof( szPassword2 ), "fegG-24qw##4dfe52%3*" );
				}

				if( strcmp( szLowerFileName, "rs" ) != 0 && strcmp( szLowerFileName, "map" ) != 0 ) // 낱개 파일
				{
					if( m_zip.GetFileComment(0) == "1" )
						m_zip.SetPassword( szPassword2 );
					else
						m_zip.SetPassword( szPassword1 );
				}
				else
					m_zip.SetPassword( szPassword1 );
			}

			m_zip.SetEncryptionMethod();
...
bool CZipArcDoc::AddFolder(CString szFolder, CAddInfoArray &array, CProgressData& data)
{
	...
	while (b)
	{
		...
		{
			ZIP_FILE_USIZE size;
#if defined _ZIP64 && defined _MSC_VER >= 1300
...
UINT CZipArcDoc::Rename( LPVOID pParam )
{
	...
	CZipIndexesArray* pdw = (CZipIndexesArray*)data.m_pData;
	data.Begin(0); // will be initialized later
	data.m_iType = CZipActionCallback::cbRename; // it is only initialized by ZipArchive library if the length of the filenames is different
	data.GetDoc()->GetView()->PostMessage(WM_SHOWHIDEVIEW, 0);
	TRY
	{
	...
	for (i = 0; i < iCount; i++)
	{
		...
		TRY
		{
			...
			if (!bSkip)
			{
				...
				// IO
				CString szTemp = data.m_szFileInZip;
				szTemp.MakeLower();
				szTemp.MakeReverse();

				if( szTemp.Find("ini." ) == 0 && zip.GetFileComment( nIndex ) == "1" )
				{
					CString szDecryptFileName = szExtractTo;
					szDecryptFileName += "\\";
					szDecryptFileName += data.m_szFileInZip;
					DecryptFile( szDecryptFileName );
				}
				//
			}
...
void CZipArcDoc::EncryptDecryptData( OUT char *szResultData, IN const int iResultSize, IN const char *szSourceData, IN const int iSourceSize, IN bool bPassword )
{
...
bool CZipArcDoc::DecryptFile( const CString &rszFileName )
{
	// read
	CFile file;
	...
	// enrypt
	EncryptDecryptData( pData, iSize, pData, iSize, false );
