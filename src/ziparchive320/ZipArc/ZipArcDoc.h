	...
	struct CProgressData : public CZipActionCallback
	{
		...
		CProgressData(CZipArcDoc* pDoc):m_pDoc(pDoc)
		{
			const int iActions[] = {CZipActionCallback::cbNothing, CZipActionCallback::cbAdd,CZipActionCallback::cbAddTmp,CZipActionCallback::cbAddStore,CZipActionCallback::cbExtract,CZipActionCallback::cbDeleteCnt,
				CZipActionCallback::cbDelete,CZipActionCallback::cbTest,CZipActionCallback::cbSave, cbCounting, CZipActionCallback::cbRename, CZipActionCallback::cbMoveData, CZipActionCallback::cbGet};
		...
		int GetMax()
		{
			...
			if (m_bInit) // parent frame has received it
				m_bInit = false; 
			if (IsSubAction() || m_iType == CZipActionCallback::cbDelete || m_iType == CZipActionCallback::cbRename)
		...
		bool IsStopOnly()
		{
			...
			return m_iType == CZipActionCallback::cbDeleteCnt || m_iType == CZipActionCallback::cbSave && m_pDoc->GetSegmMode() == 0
				|| m_iType == cbCounting;
		}
		bool IsAbortOnly()
		{			
			return m_iType == CZipActionCallback::cbDelete || m_iType == CZipActionCallback::cbSave && m_pDoc->GetSegmMode() != 0 || m_iType == CZipActionCallback::cbRename || m_iType == CZipActionCallback::cbMoveData;
		}
...
public:
	...
	bool IsModifyingInProgress()
	{
		return m_progress.IsActive() && (m_progress.m_iType == CZipActionCallback::cbAdd || 
			...
			|| m_progress.m_iType == CZipActionCallback::cbRename || m_progress.m_iType == CZipActionCallback::cbMoveData || m_progress.m_iType == CZipActionCallback::cbGet);
	}
...
protected:
	...
	static bool DecryptFile( const CString &rszFileName );
public:
	afx_msg void OnNextsearch();
};
